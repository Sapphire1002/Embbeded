...
def main(path):
    """
    function:
        main(path): 讀取圖片做 watershed 以及影像處理
    ...
    st = time.time()
    ori_img, gray, frame_pre = preprocess(ori_img)  # preprocess
    ...
    res, last = handle_watershed(ori_img, markers)  # handle watershed
    end = time.time()
