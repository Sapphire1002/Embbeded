...
def Segmentation(im): #輸入RGB IMAGE
    ...
    grade_YR = 25; grade_YG = 29; grade_YB = 100 #範圍等級
    white_R  = 220; white_G  = 224; white_B  = 223 #改
    grade_WR = 25; grade_WG = 29; grade_WB = 28 #範圍等級
     # -----------------------------------------------
    # YellowLine
    ret,thresh_YR_0 = cv2.threshold(im_BGR[2],yellow_R-grade_YR,255,cv2.THRESH_BINARY)
    ret,thresh_YR_1 = cv2.threshold(im_BGR[2],yellow_R+grade_YR,255,cv2.THRESH_BINARY_INV)
    thresh_YR_0[thresh_YR_1==0] = 0
    ret,thresh_YG_0 = cv2.threshold(im_BGR[1],yellow_G-grade_YG,255,cv2.THRESH_BINARY)
    ret,thresh_YG_1 = cv2.threshold(im_BGR[1],yellow_G+grade_YG,255,cv2.THRESH_BINARY_INV)
    thresh_YG_0[thresh_YG_1==0] = 0
    ret,thresh_YB_0 = cv2.threshold(im_BGR[0],yellow_B-grade_YB,255,cv2.THRESH_BINARY)
    ret,thresh_YB_1 = cv2.threshold(im_BGR[0],yellow_B+grade_YB,255,cv2.THRESH_BINARY_INV)
    thresh_YB_0[thresh_YB_1==0] = 0
    thresh_YR_0[thresh_YG_0 == 0] = 0
    thresh_YR_0[thresh_YB_0 == 0] = 0
    # -----------------------------------------------
    # WhiteLine
    ret,thresh_WR_0 = cv2.threshold(im_BGR[2],white_R-grade_WR,255,cv2.THRESH_BINARY)
    ret,thresh_WR_1 = cv2.threshold(im_BGR[2],white_R+grade_WR,255,cv2.THRESH_BINARY_INV)
    thresh_WR_0[thresh_WR_1==0] = 0
    ret,thresh_WG_0 = cv2.threshold(im_BGR[1],white_G-grade_WG,255,cv2.THRESH_BINARY)
    ret,thresh_WG_1 = cv2.threshold(im_BGR[1],white_G+grade_WG,255,cv2.THRESH_BINARY_INV)
    thresh_WG_0[thresh_WG_1==0] = 0
    ret,thresh_WB_0 = cv2.threshold(im_BGR[0],white_B-grade_WB,255,cv2.THRESH_BINARY)
    ret,thresh_WB_1 = cv2.threshold(im_BGR[0],white_B+grade_WB,255,cv2.THRESH_BINARY_INV)
    thresh_WB_0[thresh_WB_1==0] = 0
    thresh_WR_0[thresh_WG_0 == 0] = 0
    thresh_WR_0[thresh_WB_0 == 0] = 0
    # -----------------------------------------------
    Combine_YW = thresh_WB_0.copy() #黃色與白色合併找Contours
    Combine_YW[thresh_WR_0==255] = 255
    Combine_YW[thresh_YR_0==255] = 255
    Segmentation_out[thresh_WR_0==255,:] = (0,0,255)
    Segmentation_out[thresh_YR_0==255,:] = (0,0,255)
    # -----------------------------------------------
    ret, contours, hierarchy = cv2.findContours(Combine_YW,cv2.RETR_EXTERNAL,cv2.CHAIN_APPROX_SIMPLE)
    cv2.imshow("Combine_YW",cv2.pyrDown(Combine_YW))
    cv2.imshow("Segmentation_out",cv2.pyrDown(Segmentation_out)) #紅色覆蓋
    return contours,Combine_YW,Segmentation_out
# -----------------------------------------------------------
#計算Foe距離
def CalFoe(im,lines,contours_buf,thresh_WR_0,mag) : #輸入RGB IMAGE
    ...
    # -----------------------------------------------   
    for c in contours :
        if (cv2.contourArea(c) > 500): #設定Contours面積範圍
            ...
            for (x1,y1),(x2,y2) in lines :
                if (x+w > x1 > x) and (y+h > y1 > y) : #contours範圍
                    if (Combine_YW[y1][x1] == 255) : #有在白色範圍內
...
# path_VideoWriter = 'D:\\Special_topic_project\\Project\\dist_Foe_6_CalFoe.mp4'
# path_VideoWriter = 'D:\\Special_topic_project\\Project\\dist_Foe_6_Segmentation.mp4'
# videoWriter = cv2.VideoWriter(path_VideoWriter,fourcc,30,(width,height),False) #黑白
# videoWriter = cv2.VideoWriter(path_VideoWriter,fourcc,30,(width,height)) #RGB
# -----------------------------------------------------------
#設定初始狀態
...
while (cap.isOpened()):
    ...
    if ret==True :
        ...
        # ---------------------------------------------------------------
        contours, Combine_YW ,Segmentation_out= Segmentation(im)
        CalFoe_buf = CalFoe(im,lines_buf,contours,Combine_YW,mag) #Foe判斷
        # ---------------------------------------------------------------
        if show_line:
        ...
        if write_image:
            ...
            # cv2.imwrite('Segmentation_out.jpg', Segmentation_out) #輸出圖片
            None
        if write_video:
            # videoWriter.write(CalFoe_buf) #輸出影片
            # videoWriter.write(Segmentation_out) #輸出影片
            None
        # ---------------------------------------------------------------
